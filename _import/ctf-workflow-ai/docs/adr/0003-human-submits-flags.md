...
**Amended by the two-state notes schema** (issue #1): `notes.md` carries only `unsolved` or `solved`. The `in-progress` state named above belongs to the superseded orchestration design (ADR-0004/ADR-0005); under the final schema a rejected flag simply returns the note to `unsolved`.
